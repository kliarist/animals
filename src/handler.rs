...
pub async fn find_by_id(State(app_state): State<Arc<Mutex<AppState>>>, Path(id): Path<i8>) -> (StatusCode, Json<Value>) {
...
pub async fn create(app_state: State<Arc<Mutex<AppState>>>, Json(dto): Json<AnimalRequestDto>) -> (StatusCode, Json<i8>) {
    return (StatusCode::CREATED, Json(app_state.lock().await.animal_service.save(dto)))
}

pub async fn delete_by_id(State(app_state): State<Arc<Mutex<AppState>>>, Path(id): Path<i8>) -> StatusCode {
    return app_state.lock().await.animal_service
        .delete_by_id(id)
